...
function handleResponseNoBody(error, response, body, callback) {
    ...
    if (body && body.ocs && body.ocs.meta) { // Response is well-formed
        let statuscode = body.ocs.meta.statuscode;
...
// Helper function which takes a user callback awaiting an error message and a body
function handleResponseBody(error, response, body, callback) {
    if (error) {
        callback(error, body);
        return;
    }
    if (response.statusCode != 200) {
        callback({code: response.statusCode, message: response.statusMessage}, body);
        return;
    }
    body = JSON.parse(body);
    if (body && body.ocs && body.ocs.meta) {
        let statuscode = body.ocs.meta.statuscode;
        if (statuscode != 100) { // Status code returned by server isn't 100 (OK)
            callback({code: statuscode, message: body.ocs.meta[0].message[0]}, body);
            return;
        }
        else
            callback(null, body);
        ...
        callback({code: 0, message: "The server said everything was fine but returned a malformed body. This should never happen."}, body);
    }
}
...
users.list = (options, callback) => {
    ...
    request.get({
        ...
        handleResponseBody(error, response, body, callback);
    });
}
...
users.getGroups = (username, callback) => {
    let url = baseUrl + "/" + username + "/groups";
    request.get({
        url : url,
        headers : config.headers,
    }, (error, response, body) => {
